...
- [ ] T013 [P] [US1] Create `core/build.gradle.kts` — apply `kotlin-jvm-convention`; no external dependencies (only stdlib + coroutines from convention)
...
- [ ] T020 [US1] Create `server/api/build.gradle.kts` — apply `ktor-service-convention`; add `implementation(projects.server.domain)`, `implementation(projects.core.models)`, `implementation(projects.core)`; add Ktor test engine + Kotest to `testImplementation`
...
- [ ] T025 [P] [US1] Create `server/app/src/main/resources/logback.xml` — JSON appender using `net.logstash.logback.encoder.LogstashEncoder`; includes `requestId` MDC field; production format; create `server/app/src/dev/resources/logback-dev.xml` — `PatternLayoutEncoder` with coloured human-readable output for local dev
- [ ] T026 [US1] Create `server/app/src/dev/kotlin/dev/kodex/server/dev/DevModule.kt` — dev-only Koin module + registers `get("/dev/ping") { call.respond("pong") }` debug route; only compiled when `devRun` task is used; NOT included in `installDist` production build

### sandbox-runner/ modules

- [ ] T027 [P] [US1] Create `sandbox-runner/executor/build.gradle.kts` — apply `kotlin-jvm-convention`; add `docker-java` from version catalog; create `src/main/kotlin/dev/kodex/sandbox/executor/.gitkeep` placeholder
- [ ] T028 [US1] Create `sandbox-runner/app/build.gradle.kts` — apply `ktor-service-convention`; add `implementation(project(":sandbox-runner:executor"))`; create `sandbox-runner/app/src/main/kotlin/dev/kodex/sandbox/Application.kt` — minimal Ktor server skeleton (port from env, single `/health` route returning 200)
